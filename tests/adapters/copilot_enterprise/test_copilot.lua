...
T["Copilot Enterprise adapter"] = new_set({
  hooks = {
    pre_case = function()
      adapter = require("codecompanion.adapters").resolve("copilot_enterprise")
    end,
  },
})

T["Copilot Enterprise adapter"]["it can form messages to be sent to the API"] = function()
  local messages = { {
...
T["Copilot Enterprise adapter"]["it can form tools to be sent to the API"] = function()
...
T["Copilot Enterprise adapter"]["Streaming"] = new_set()

T["Copilot Enterprise adapter"]["Streaming"]["can output streamed data into the chat buffer"] = function()
  local output = ""
...
T["Copilot Enterprise adapter"]["Streaming"]["can process tools"] = function()
  local tools = {}
...
T["Copilot Enterprise adapter"]["No Streaming"] = new_set({
  hooks = {
    pre_case = function()
      adapter = require("codecompanion.adapters").extend("copilot_enterprise", {
        opts = {
          stream = false,
        },
      })
...
T["Copilot Enterprise adapter"]["No Streaming"]["can output data into the chat buffer"] = function()
...
T["Copilot Enterprise adapter"]["No Streaming"]["can process tools"] = function()
...
T["Copilot Enterprise adapter"]["No Streaming"]["can output for the inline assistant"] = function()
  local data = vim.fn.readfile("tests/adapters/stubs/copilot_no_streaming.txt")
  data = table.concat(data, "\n")
