...
T["inline diff with buffer layout keeps window open on accept"] = function()
  local result = with_mocks({
    wait_behavior = [[
      -- Resolve immediately as accepted
      cb({ accepted = true })
    ...
    body = [[
      -- Configure to use inline provider with buffer layout
      local cfg = require("codecompanion.config")
      cfg.display = cfg.display or {}
      cfg.display.diff = {
        provider = "inline",
      ...
      -- Create an alternate buffer to switch back to
      local alt_buf = vim.api.nvim_create_buf(true, false)
      vim.api.nvim_buf_set_lines(alt_buf, 0, -1, false, { "alternate buffer" })
      vim.cmd("buffer " .. alt_buf)

      local responded = {}
      local chat = { bufnr = alt_buf }
      local request = {
        tool_call = {
          toolCallId = "tc-buffer-layout",
          kind = "edit",
          title = "Apply changes",
          status = "pending",
          content = { { type = "diff", path = "file.txt", oldText = "old", newText = "new" } },
      ...
      -- After cleanup, check if window is still valid and switched to alternate buffer
      local diff_bufnr = _G.__CAPT.inline_new.bufnr
      local diff_buf_deleted = not vim.api.nvim_buf_is_valid(diff_bufnr)
      ...
      return {
        ...
        canceled = responded.canceled,
        accept_called = _G.__CAPT.accept_called == true,
        diff_buf_deleted = diff_buf_deleted,
        switched_to_alt = current_buf == alt_buf,
      }
    ]],
  })
  ...
  h.eq(false, result.canceled)
  h.eq(true, result.accept_called)
  h.eq(true, result.diff_buf_deleted)
  h.eq(true, result.switched_to_alt)
end

T["inline diff with float layout closes window on reject"] = function()
  local result = with_mocks({
    wait_behavior = [[
    ...
    body = [[
      -- Configure to use inline provider with float layout
      local cfg = require("codecompanion.config")
      cfg.display = cfg.display or {}
      cfg.display.diff = {
        provider = "inline",
      ...
      -- Capture the window that was created
      local diff_winnr
      local orig_open_win = vim.api.nvim_open_win
      vim.api.nvim_open_win = function(...)
        diff_winnr = orig_open_win(...)
        return diff_winnr
      end
      ...
      -- After cleanup with float layout, window should be closed
      local diff_bufnr = _G.__CAPT.inline_new.bufnr
      local diff_buf_deleted = not vim.api.nvim_buf_is_valid(diff_bufnr)
      local diff_win_closed = diff_winnr and not vim.api.nvim_win_is_valid(diff_winnr)
      ...
      return {
        ...
        reject_called = _G.__CAPT.reject_called == true,
        diff_buf_deleted = diff_buf_deleted,
        diff_win_closed = diff_win_closed,
      }
    ]],
  })
  ...
  h.eq(true, result.reject_called)
  h.eq(true, result.diff_buf_deleted)
  h.eq(true, result.diff_win_closed)
end
