...
T["PromptBuilder flags has_diff when content includes diff"] = function()
  local result = child.lua([[
    local Connection = require("codecompanion.acp")
    ...
    local conn = Connection.new({ adapter = adapter })
    conn.session_id = "test-session-4"

    local pb = conn:prompt({ { type = "text", text = "permission?" } })

    local seen = { has_diff = false, kinds = {} }
    pb:on_permission_request(function(req)
      seen.has_diff = req.has_diff
      for _, opt in ipairs(req.options or {}) do
        seen.kinds[opt.kind] = true
      end
    end)
    ...
    pb:_handle_permission_request(21, params)
    return { has_diff = seen.has_diff, allow_once = seen.kinds["allow_once"] or false, reject_once = seen.kinds["reject_once"] or false }
  ]])

  h.eq(true, result.has_diff)
  h.eq(true, result.allow_once)
  h.eq(true, result.reject_once)
end
