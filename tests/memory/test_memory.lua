...
local T = new_set({
  hooks = {
    ...
    post_case = function()
      child.lua([[pcall(vim.fn.delete, _G.TEST_CWD, "rf")]])
    end,
...
T["Memory"]["indexes nested memories as paths the tool accepts back"] = function()
  ...
  }, child.lua([[return require("codecompanion.memory").index()]]))
end

T["Memory"]["prompt lists every memory"] = function()
  add_memories()

  local prompt = child.lua([[return require("codecompanion.memory").prompt()]])
