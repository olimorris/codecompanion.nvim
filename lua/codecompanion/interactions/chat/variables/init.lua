...
local function resolve(chat, var_config, params, target)
  ...
  return require("codecompanion.interactions.chat.variables.user")
    .new({
      Chat = chat,
      config = var_config,
      params = params,
...
function Variables:replace(message, bufnr)
  for var, _ in pairs(self.vars) do
    ...
    if var:match("^buffer") then
      message = require("codecompanion.interactions.chat.variables.buffer").replace(CONSTANTS.PREFIX, message, bufnr)
    else
