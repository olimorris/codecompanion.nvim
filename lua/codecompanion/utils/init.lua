...
function M.regex_replace(str, regex_pattern, replacement)
...
---@return number?,number? start_pos The start position of the match, or nil if not found
---@return nil
function M.regex_find(str, regex_pattern)
  local regex = vim.regex(regex_pattern)
  if not regex then
    return nil
  end
