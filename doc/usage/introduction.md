...
You can [review an LLM or agent's changes](/usage/code-review) like a pull request. `:CodeCompanionCodeReview` opens a review window with every changed file and its hunks alongside the code. You can step through them, accept or revert each one, leave in place comments and then share the review in a chat buffer with the [#{code_review}](/usage/chat-buffer/editor-context#code-review) context.
