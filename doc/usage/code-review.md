...
> [!IMPORTANT]
> Snapshots are produced against an index owned by CodeCompanion, so `git add` never runs against the user's. This means a user's staged changes and anything that's pushed are unaffected by a code review
