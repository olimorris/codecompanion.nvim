...
As outlined by Andrew Ng in [Agentic Design Patterns Part 3, Tool Use](https://www.deeplearning.ai/the-batch/agentic-design-patterns-part-3-tool-use), LLMs can act as agents by leveraging external tools. Andrew notes some common examples such as web searching or code execution that have obvious benefits to LLMs.

This plugin has added support for a `code_runner` tool which can take the code generated by an LLM and execute it in a Docker container, before posting the output back to the chat buffer for further feedback and iteration.

You can create your own tools by following the [TOOLS](TOOLS.md) guide.
